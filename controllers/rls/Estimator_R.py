...
class RLS_EHGO_R:
      ...
      def EHGO_R(self, EHGO_R_input):
            ...
            phi2_ob = phi2_ob_ + self.T*(-Ac[0] + Bc[0] + Mdc[0] + sig_phi_ + self.r1/self.epsilonr*(phi2_ - phi2_ob_))
            theta2_ob = theta2_ob_ + self.T*(-Ac[1] + Bc[1] + Mdc[1] + sig_theta_ + self.r1/self.epsilonr*(theta2_ - theta2_ob_))
            psi2_ob = psi2_ob_ + self.T*(-Ac[2] + Bc[2] + Mdc[2] + sig_psi_ + self.r1/self.epsilonr*(psi2_ - psi2_ob_))

            sig_phi = sig_phi_ + self.T*(self.r2/self.epsilonr**2*(phi2_ - phi2_ob_))    
            sig_theta = sig_theta_ + self.T*(self.r2/self.epsilonr**2*(theta2_ - theta2_ob_))
            sig_psi = sig_psi_ + self.T*(self.r2/self.epsilonr**2*(psi2_ - psi2_ob_))
